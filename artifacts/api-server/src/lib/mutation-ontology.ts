...
export const MUTATION_ONTOLOGY: Record<CanonicalCommandName, CanonicalMutationCommand> = {
  ...
  FATIGUE_MANAGEMENT: {
    ...
    category: "difficulty_adjustment",
    description: "Reduce accumulated fatigue by trimming accessory volume and softening intensity while preserving movement balance.",
    defaultScope: "week",
    aliases: ["fatigue_management"],
    minimumStructuralChanges: 1,
    antiPatterns: ["removing primary lifts", "cutting all accessories without cause", "collapsing sessions to rest days", "randomly removing exercises without preserving pattern balance"],
    aiDirective: "Reduce accumulated fatigue intelligently — preserve all movement patterns and session balance. Drop 1 set from accessories (never primaries). Extend rest periods by 30-60s. Add intensity cue: 'work to ~70-75% effort, stop well short of failure'. Do NOT remove entire exercises or collapse sessions. Add a brief coaching note explaining the fatigue-management rationale. Maintain the training stimulus — this is a volume/intensity reduction, not a rest day.",
  },
  RECOVERY_FOCUS: {
    name: "RECOVERY_FOCUS",
    category: "mobility_recovery",
    description: "Apply a structured deload week: intelligently reduce volume and intensity while preserving movement patterns and session balance.",
    defaultScope: "week",
    aliases: ["recovery_focus"],
    minimumStructuralChanges: 1,
    antiPatterns: ["full session conversion to rest day without cause", "removing primary work entirely", "collapsing multiple sessions into one", "removing all accessories from a session"],
    aiDirective: "Apply a structured deload across all sessions in the week: reduce accessory sets by 1, keep primary lifts at 2-3 sets at ~60-70% effort (no maximal attempts). Instruct: terminate sets with 3+ reps in reserve (RIR). Add per-session coaching notes emphasizing movement quality over load. Do NOT collapse sessions to rest days — maintain the training stimulus while reducing demand. Preserve all movement pattern categories across the week (push, pull, hinge, knee-dominant, core).",
  },
  MOBILITY_SUPPORT: {
