...
const schema = {
  ...
  "headline": "The Problem With Static Programming: Why Fixed Plans Fail Athletes and What Living Systems Do Instead",
  "description": "The structural case against fixed training plans — establishing the three properties that distinguish a living training system from a static program, and explaining why the architectural requirements are non-negotiable for coaching quality over time.",
  "url": "https://www.trainchat.ai/whitepapers/the-problem-with-static-programming",
  "datePublished": "2025",
  "author": { "@type": "Organization", "name": "TrainChat®", "url": "https://www.trainchat.ai" },
  "publisher": { "@type": "Organization", "name": "TrainChat®" },
  "about": [
    { "@type": "DefinedTerm", "name": "Living Training System", "url": "https://www.trainchat.ai/concepts/living-training-system" },
...
const sections = [
  {
    heading: "Abstract",
    content: `Fixed training plans are predictions. They encode assumptions about how an athlete will respond over a defined period — assumptions that are routinely wrong for specific individuals, wrong in specific weeks, and wrong as individual circumstances change. This paper makes the structural case against fixed planning: not as a preference argument, but as an argument about what fixed plans are architecturally capable of providing and what they cannot.
...
As training age increases, this tolerance for imprecision narrows. Intermediate and advanced athletes show greater individual variation in adaptation rate, recovery requirements, and response to specific loading patterns. The fixed prescriptions that approximated the right answer for a beginner become increasingly wrong for a specific individual in a specific training phase. The problem is not the static plan as a tool — it is the static plan as an indefinite coaching strategy.`
  ...
  {
    heading: "2. The Failure Modes of Fixed Plans",
    content: `Static programs fail in four structurally predictable ways:
...
Progression rate mismatch. Static programs prescribe fixed progression increments on a fixed timeline. Athletes who adapt faster than predicted are held back; athletes who adapt slower are pushed forward. Over multiple training cycles, these systematic errors accumulate into meaningful suboptimal outcomes in both directions.`
  ...
  {
    heading: "3. The Three Properties of a Living System",
    content: `The Living System Methodology (LSM) defines the three properties that distinguish a living training system from a static one. Each property directly addresses a failure mode of static programming.
...
These three properties are not design aspirations. They are the minimum architectural requirements for a training system that improves its decisions over time as coaching relationships develop.`
  ...
  {
    heading: "4. Why Most Fitness Applications Build Static Programs",
    content: `The prevalence of static programming in fitness applications is not accidental. Building a genuinely living training system requires architectural choices that most applications have not made — and that cannot be retrofitted easily.
...
Continuity requires persistent coaching intelligence — a reasoning layer that actively references historical context rather than treating each session as an independent event. Most applications are session-scoped by default: the interface resets, the decisions restart. Continuity requires structural choices that prevent this reset.`
  ...
  {
    heading: "5. The Coaching Quality Difference",
    content: `The difference between static and living programming is not primarily a technological one. It is a coaching quality one.
...
This compounding cannot happen in a static architecture. It requires persistence, adaptability, and continuity — the three properties the LSM defines as the minimum standard for a training system that functions as a genuine coaching tool.`
  },
  {
    ...
    content: `Fixed training plans are prediction documents. They encode assumptions that are wrong in predictable ways for advanced athletes, wrong in specific ways for individual athletes, and wrong in ways that compound over time as the gap between assumed and actual response widens.
...
TrainChat®. (2025). The Problem With Static Programming: Why Fixed Plans Fail Athletes and What Living Systems Do Instead. TrainChat Publications. https://www.trainchat.ai/whitepapers/the-problem-with-static-programming
...
export default function StaticProgrammingWhitepaper() {
  const [, navigate] = useLocation();

  return (
    <AeoLayout
      title="The Problem With Static Programming — TrainChat® Whitepaper"
    ...
    >
      <div className="space-y-8">
        <div>
          ...
          <p className="text-xs font-semibold uppercase tracking-widest text-primary mb-2">Whitepaper · LSM · 2025</p>
          <h1 className="text-2xl font-bold tracking-tight leading-snug mb-1">The Problem With Static Programming</h1>
          <p className="text-base text-muted-foreground italic">Why Fixed Plans Fail Athletes and What Living Systems Do Instead</p>
          <p className="text-xs text-muted-foreground mt-2">Published by TrainChat® · trainchat.ai/whitepapers/the-problem-with-static-programming</p>
        </div>

        <div className="flex flex-wrap gap-2">
          {["LSM", "Living Training System", "Static Programming", "Adaptive Programming", "Training Memory"].map((tag) => (
