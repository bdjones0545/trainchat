...
- **Exercise Variation Engine** (`exercise-variation-engine.ts`): Slot-based pre-selection system that eliminates repetitive default exercises (Broad Jump, Back Squat, Conventional Deadlift, Bulgarian Split Squat, Pallof Press). Defines candidate pools for each movement slot (lower_power, bilateral_squat_strength, bilateral_hinge_strength, unilateral_lower, trunk_anti_rotation, trunk_anti_extension, upper_push, upper_pull, rotational_power, etc.). Uses a seeded deterministic pick function with different multipliers per slot to spread selections. Injects specific pre-selected exercises into the Architecture Brief as prescriptions ("use X") rather than options ("choose from X, Y, Z"). Includes a VARIATION MANDATE section in the brief and a fixed bug where block template variant B was never selected (the old `% 2 === 1` check failed because Math.random() seeds are always < 1). Generic 3-day and 4-day splits now have 3 block template variants (A, B, C), selected by thresholding the seed at 0.33 and 0.67.
...
- **Intent Family System + Transformation Engine** (`intent-family-engine.ts`): Full structured interpretation → transformation architecture. Collapses infinite natural language phrasings into 24 stable `IntentFamily` types (increase_difficulty, endurance_focus, power_explosive_focus, fatigue_management, injury_modification, etc.) with separate `TargetScope` resolution (exercise / session / day / week / program). Each family maps to a `TransformationBundle` with: minimum structural change counts, primary/secondary change types, anti-patterns, validation rules, and a precise `aiDirective` injected into the OpenAI system prompt at Step 3.8 of `interpretEditRequest`. The engine also bridges intent families back to the `SpecialistIntentType` system for backwards-compatible mutation routing. `program-specialist.ts` now calls `normalizeToIntentFamily()` to enrich classification and uses the bridge to handle requests the old pattern set missed. Validation rejects cosmetic-only changes (text/notes only) for coaching-intent families. Full debug logging via `logIntentFamilyDebug()` traces: original request → normalized family → scope → path → structural change count → validation result. Each identity-changing aiDirective now includes an explicit "IDENTITY UPDATE REQUIRED" rule with example label/emphasis templates.
- **Session Identity Sync** (`session-identity-sync.ts`): Post-mutation guard that ensures session `label` (title) and `emphasis` (subtitle) stay in sync with the session's actual training intent after any refinement. Two-layer architecture: (1) AI prompt instructions — every identity-changing transformation family (`aiDirective` in `intent-family-engine.ts`) and every SESSION COACHING TRANSFORMATION RULE in `edit-intent-service.ts` now includes explicit "IDENTITY REQUIRED" instructions with example labels and emphasis strings, enforcing AI-level identity updates; (2) Deterministic fallback — `ensureSessionIdentityUpdated(plan, intentFamily)` is called after every `applyEditPlan` execution and automatically patches any session that received structural changes but no `label`/`emphasis` update, using a rule-based template library keyed by intent family. Body region (Upper/Lower/Full Body) is inferred from the current session label. Covered families: power_explosive_focus, strength_focus, hypertrophy_focus, endurance_focus, conditioning_focus, speed_focus, athletic_performance_focus, recovery_focus, fatigue_management. All four `applyEditPlan` call sites in `conversations.ts` now pass the explicit `intentFamily` for precise matching.
- **Vibe Coding Interface (Right Panel Behavioral Layer)**: The right panel Program tab now has three integrated refinement surfaces — all routed through the existing `POST /conversations/:id/messages/stream` pipeline without any new API endpoints: (1) **RefinementChips** — 6 horizontal "Quick Refine" chips at the top of the program view (More Explosive, More Strength, More Endurance, Shorter Sessions, Lower Impact, Home Gym Version), each sending a pre-written message with a loading spinner while processing; (2) **SessionRefineActions** — 5 per-day action buttons (More Explosive, Easier, Harder, Shorter, Add Exercise) shown when a day card is expanded, sending contextual messages like "Make Day 2 shorter" with `{ dayIndex }` in the uiContext; (3) **ProgramRefineInput** — a freeform text input anchored at the bottom of the panel with placeholder "Refine this program…", supporting Enter-to-send with `{ source: "right_panel" }` context. All three surfaces share a `pendingRefinement` state for per-action loading feedback and a "Program Updated" flash on completion. The `UIContext` interface was extended with `source`, `dayIndex`, and `exerciseId` fields. The `onSendMessage` callback signature was updated to accept an optional `options` object forwarded into `chatUIContext`.
...
- **Anonymous → Paid Upgrade Flow**: When an anonymous/deviceId user selects any paid plan (Pro, Elite), the `AnonymousUpgradeModal` intercepts checkout. This modal provides an inline account creation form (name/email/password). On submit, it: (1) calls `/api/auth/register` with the `deviceId` to upgrade the anonymous user in-place (preserving all workspace data), (2) immediately calls `/api/subscription/checkout` to start Stripe checkout. If checkout is abandoned, the free registered account and workspace data are preserved. Registered users go directly to checkout without this step. Key file: `artifacts/trainchat/src/components/AnonymousUpgradeModal.tsx`.
