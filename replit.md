...
- **Hierarchical Program Generation System**: A top-down, 5-layer planning architecture replacing flat session generation:
  ...
  - **Block Phase Banner (UI)**: The `LiveProgramPanel` Program tab renders a `BlockPhaseBanner` component above the day cards when `blockMetadata` is present. The banner displays the block display name, mission statement, primary adaptation, and volume/intensity profiles with block-type-specific color coding (orange = power_conversion, violet = hypertrophy_support, blue = strength_emphasis, green = work_capacity, amber = re_entry_resilience). The banner receives `(activeSystem as any)?.metadata` passed from `chat.tsx` — no extra API calls needed since the active system query already returns the full row.
...
- **Exercise Variation Engine**: A slot-based pre-selection system for exercise variations, designed to reduce repetition and inject specific prescriptions. Now receives hierarchical block context (blockType + weekRole) from the monthly/weekly planners to enrich intent scoring.
